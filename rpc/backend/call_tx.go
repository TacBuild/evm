...
// DoTacSimulate performs a simulated call operation through the evmtypes. On top
// of DoCall it returns the emitted logs and the gas estimation of the call.
//
// Unlike DoCall it deliberately skips handleRevertError: a revert is part of the
// simulation result (reported through VmError and Ret), not a JSON-RPC error.
func (b *Backend) DoTacSimulate(
	...
	// return result as is, reverts included
	return res, nil
}
