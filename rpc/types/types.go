...
// TacSimulateResult is the JSON-RPC result of the custom `tac_simulate` method.
...
// ToRPCTypeLogs casts the proto compatible logs to the JSON-RPC log type.
func ToRPCTypeLogs(logs []*evmtypes.Log) []ethtypes.Log {
	ethLogs := evmtypes.LogsToEthereum(logs)

	rpcLogs := make([]ethtypes.Log, len(ethLogs))
