...
// TestSetBalanceRejectsModuleAccounts verifies the EVM balance-reconciliation
// path (SetBalance mints/burns the delta to reach a target) must never target a
// module account: the call must error and leave the balance unchanged.
...
// TestSetBalanceAllowsEOA confirms the guard does not regress ordinary (EOA)
