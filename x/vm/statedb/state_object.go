...
func (s *stateObject) SubBalance(amount *uint256.Int) uint256.Int {
	...
	// Guard against underflow: subtracting more than the (spendable) balance would
	// wrap the uint256 to a huge value and mint coins at commit. Fail loudly and
	// deterministically instead. (Mirrors cosmos/evm #1176.)
	balance := s.Balance()
